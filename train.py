...
def create_offset_gt(image, offset):
    ...
    id_coords = torch.from_numpy(id_coords).to(image.device)
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    # For rendering, we need CUDA due to rasterization requirements
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # Loss - ensure all tensors are on the same device as rendered image
        ...
        # Apply mask to both rendered and ground truth images
        mask = viewpoint_cam.original_mask.to(image.device)
        ...
        with torch.no_grad():
            # Early pruning of Gaussians based on mask projection at iteration 100
            if iteration == 100:
            ...
            # Log and save
            elapsed_time = iter_start.elapsed_time(iter_end) if iter_start is not None else 0.0
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed_time, testing_iterations, scene, render, (pipe, background, dataset.kernel_size))
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    # Apply mask for evaluation
                    mask = viewpoint.original_mask.to(image.device)
                    masked_image = image * mask
                    masked_gt_image = gt_image * mask
                    
                    if tb_writer and (idx < 5):
                        tb_writer.add_images(config['name'] + "_view_{}/render".format(viewpoint.image_name), masked_image[None], global_step=iteration)
                        if iteration == testing_iterations[0]:
                            tb_writer.add_images(config['name'] + "_view_{}/ground_truth".format(viewpoint.image_name), masked_gt_image[None], global_step=iteration)
